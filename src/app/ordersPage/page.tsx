...
export default function OrdersPage() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      const snap = await getDocs(collection(db, "userProfilesAndOrderStatus"));
  ...
  const applyFilters = (
    ...
    if (city) {
      filtered = filtered.filter((user) => user.profile?.[0]?.city === city);
    }
  ...
  const uniqueCities = [...new Set(users.map((u) => u.profile?.[0]?.city).filter(Boolean))];
    ...
    <div style={{ padding: "24px" }}>
      ...
      <Row gutter={[16, 16]}>
        {filteredUsers.length === 0 ? (
          <Empty description="No matching users found" style={{ margin: "auto" }} />
        ) : (
            ...
            return (
              <Col key={user.id} xs={24} sm={12} md={8}>
                ...
                >
                  ...
                  <p>
                    <Text strong>🛒 Orders: {user.orders?.length || 0}</Text>
                  </p>
                </Card>
              </Col>
            );
          ...
          >
                  ...
                  <Card
                    ...
                    cover={
                      <img
                        ...
                        style={{ height: 100, objectFit: "contain", padding: 8 }}
                      />
                    }
                  >
                    <Text strong>{item.product?.productName?.en}</Text>
                    <p>Type: {item.product?.productType?.en}</p>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <Select
                ...
                value={order.status}
                onChange={(val) => updateStatus(selectedUser.id, order.id, val)}
              >
                <Option value="Processing your order">Processing</Option>
                <Option value="Packed">Packed</Option>
                <Option value="Shipped">Shipped</Option>
